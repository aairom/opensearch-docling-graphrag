...
if lsof -Pi :8501 -sTCP:LISTEN -t >/dev/null 2>&1; then
    ...
    if podman ps --format "{{.Names}}" | grep -q "docling-app"; then
        ...
        echo -e "   ${GREEN}REST API:${NC}      http://localhost:8000/api/docs"
        echo -e "   ${GREEN}GraphQL:${NC}       http://localhost:8000/api/graphql"
        echo -e "   ${GREEN}WebSocket:${NC}     ws://localhost:8000/api/ws"
        ...
        echo ""
        echo -e "📝 Logs:"
        echo -e "   ${GREEN}Streamlit:${NC}   $CONTAINER_CMD logs -f docling-app"
        echo -e "   ${GREEN}API:${NC}         $CONTAINER_CMD logs -f docling-api"
        echo ""
...
if ps -p $STREAMLIT_PID > /dev/null; then
    ...
    echo ""
    echo -e "📝 Logs:"
    echo -e "   ${GREEN}Streamlit:${NC}   tail -f logs/streamlit.log"
    echo -e "   ${GREEN}API:${NC}         $COMPOSE_CMD logs -f api"
